  ...
  <q-page padding>
    <div class="q-pa-md q-gutter-sm">
      <q-breadcrumbs>
        ...
        <q-breadcrumbs-el label="Candidatos a baja" icon="delete_sweep" />
      </q-breadcrumbs>
    </div>
    ...
    <div v-if="modulo && modulo.leer">
      ...
      >
        <template v-slot:top>
          <h1 class="text-h6">Expedientes con vigencia documental prescrita</h1>
          <q-space />
          <q-btn flat dense color="primary" icon="picture_as_pdf" label="Descargar borrador de dictamen"
            @click="descargarBorrador" class="q-mr-sm" />
          <q-btn flat dense color="secondary" icon="arrow_forward" label="Ir a bajas documentales"
            :to="{ name: 'bajaDocumental' }" />
          <q-btn flat round dense icon="refresh" @click="recargar" aria-label="Actualizar" />
        </template>

        <template v-slot:no-data>
          <div class="full-width row flex-center q-pa-md text-grey-7">No hay expedientes candidatos a baja.</div>
        </template>
      </q-table>
    </div>
    <SinPermisoBanner v-else modulo="Candidatos a baja" />
  </q-page>
</template>
...
import { useCandidatosBajaStore } from "../../../stores/candidatos_baja_store";
import SinPermisoBanner from "../../../components/SinPermisoBanner.vue";

const $q = useQuasar();
const authStore = useAuthStore();
const store = useCandidatosBajaStore();
const { modulo } = storeToRefs(authStore);
const { candidatos } = storeToRefs(store);
const siglas = "AI-BD-CANDIDATOS";
...
const descargarBorrador = async () => {
  $q.loading.show();
  const resp = await store.descargarBorradorDictamen();
  $q.loading.hide();
  if (!resp.success) $q.notify({ type: "negative", message: resp.data });
};
