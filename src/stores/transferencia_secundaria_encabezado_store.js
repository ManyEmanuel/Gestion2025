...
export const useTransferenciaSecundariaEncabezadoStore = defineStore('TransferenciaSecundariaEncabezado', {
  ...
  actions: {
    ...
    async loadEncabezados() {
      try {
        this.isLoading = true
        const resp = await api.get('/Archivo/TransferenciasSecundariasEncabezados')
        if (resp.status == 200) {
    ...
    async createTransferencia(encabezado) {
      try {
        const resp = await api.post('Archivo/TransferenciasSecundariasEncabezados', encabezado)
        if (resp.status == 200) {
    ...
    async afectar(id) {
      try {
        const resp = await api.get(`/Archivo/TransferenciasSecundariasEncabezados/Afectar/${id}`)
        if (resp.status == 200) {
          const { success, data } = resp.data
          return { success, data }
        } else {
